...
const SELECT_SCAN: &str = "SELECT id, target, kind, engine, status, attempts, max_attempts, \
                           error, created_at, started_at, finished_at FROM scans WHERE id = $1";
...
impl ScanRepository for PostgresScanRepository {
    async fn insert(&self, scan: &NewScan) -> Result<(), AppError> {
        sqlx::query(
            "INSERT INTO scans (id, target, kind, engine, max_scripts, max_attempts, created_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7)",
        )
        .bind(&scan.id)
        .bind(&scan.target)
        .bind(scan.kind.as_str())
        .bind(scan.engine.as_str())
